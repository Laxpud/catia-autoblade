...
它是内部协议，不构成公共插件 API；协议版本随算法与完整回归证据演进。
"""
...
REQUEST_SCHEMA = "autoblade.freecad/request/v1"
RESULT_SCHEMA = "autoblade.freecad/result/v1"
...
VENDOR_MANIFEST_SHA256 = (
    "9e323a0b4c29320fb0c694519fe355021db379c4c81b25519d9ececf468d24df"
)
...
def validate_curveswb_checkout(root: Path, *, custom: bool = False) -> dict[str, str]:
    ...
    if not custom:
        ...
        try:
            ...
            if hashlib.sha256(manifest_bytes).hexdigest() != VENDOR_MANIFEST_SHA256:
                raise ProtocolError("CurvesWB manifest fingerprint mismatch")
            ...
            for item in manifest["licenses"]:
                if (
                    ...
                    raise ProtocolError("CurvesWB license fingerprint mismatch")
        except (OSError, ValueError, KeyError) as error:
            raise ProtocolError(f"Invalid CurvesWB manifest: {error}") from error
    actual: dict[str, str] = {}
    for relative, expected in CURVESWB_FILE_SHA256.items():
        ...
        if not custom and digest != expected:
            raise ProtocolError(
...
def validate_request(value: Any) -> dict[str, Any]:
    ...
    if topology not in {"sharp", "blunt"}:
        raise ProtocolError("trailing_edge_topology must be exactly 'sharp' or 'blunt'")
    ...
    for filename, raw_airfoil in airfoils.items():
        ...
        if type(airfoil["point_count"]) is not int or airfoil["point_count"] != len(
            points
        ):
            raise ProtocolError(
    ...
    writer = _mapping(modeling["step_writer"], "modeling.step_writer")
    _exact_keys(
        writer,
        {"precision_mode", "precision_mm", "surfacecurve_mode", "length_unit"},
        "modeling.step_writer",
    )
    if (
        type(writer["precision_mode"]) is not int
        or writer["precision_mode"] != 2
        or type(writer["surfacecurve_mode"]) is not int
        or writer["surfacecurve_mode"] != 1
        or isinstance(writer["precision_mm"], bool)
        or writer["precision_mm"] != 1e-7
        or writer["length_unit"] != "mm"
    ):
        raise ProtocolError(
            "STEP writer must use approved mode 2 / 1e-7 mm / surfacecurve mode 1."
        )
    _literal(
    ...
    if relative_tolerance != 1e-5:
        raise ProtocolError(
            "modeling.relative_network_tolerance must be the approved 1e-5"
        )
    if _finite(modeling["parameter_tolerance"], "modeling.parameter_tolerance") <= 0:
...
def _point(value: Any, path: str) -> tuple[float, float, float]:
    ...
    return tuple(
        _finite(coordinate, f"{path}[{index}]")
