...
| STEP precision 与矩阵报告 | writer 设置保持阶段 2 批准值；2026-09-21 用户要求后续只测量误差、由人类判断可用性 | 旧阈值转为历史记录；固定采样方法、标准报告与逐制品人工结论 |
...
- [x] 建立专用公开黄金夹具目录，只允许许可清晰的输入、CATIA STEP、manifest 和
  预计算指标；为全局忽略和构建排除设置精确例外，夹具不进入 wheel。
- [x] 覆盖单尖、单钝、不同点数多尖、多钝以及明显旋转/缩放/平移；1000 点案例
  作为稳定性/性能 smoke。
- [x] 实现 STEP 重开、单 solid、体积、包围盒、质心、站位截面和表面最大/RMS
...
验证进展（2026-09-20，阶段 4 进行中）：首个已批准 `multi-sharp-89` 公开夹具
已建立，CATIA STEP 清理后摘要为 `0eb34e4a…42dafe0`；原始/清理后 DATA 字节和
真实重开实体属性完全一致。manifest 固定来源、许可、输入/STEP 摘要、批准阈值
及 46 个站位；新增显式重建/比较工具通过交换层和跨后端门禁，全局最大
`0.357179 mm`、完整固定截面最大 `0.385172 mm`，45/46 完整覆盖及一个已知缺口
均显式记录。双平台各 231 项测试、完整检查和实际 wheel/sdist 边界校验通过。
独立 sdist/非 editable wheel 再次复现全部测量值，FreeCAD 实例 before/after
均为空，临时源码副本与环境已清理。
详细证据与复现命令见[首个公开黄金回归](../validation/golden-fixture-2026-09-20.md)。
后续进展（2026-09-21）：6 组双后端合成候选已生成并完成测量，包括单尖、
单钝、多钝及 0.5/1.7/2.5 倍缩放和明显旋转/平移；每组取得 79/79 个完整
截面，1000 点 FreeCAD 性能案例完成。Windows 桌面的统一检查包已逐文件核验；
双平台各 249 项测试和完整检查通过，真实 CATIA/FreeCAD 均无新增 owned 进程。
用户已完成 6 组人工检查，认为肉眼无可辨差异、适合当前建模与实际计算用途，
并要求以后统一只测量、形成人工判断依据。因此所有模型误差报告均不输出
阈值裁决，既有获批夹具旧阈值转为历史背景；这属于用户明确改变验收策略，
不是为了制造通过而弱化 gate。详见 [ADR-0007](../adr/0007-measure-geometry-and-defer-usability-to-humans.md)。详细数据、复现命令
和检查包位置见[候选矩阵测量与交付](../validation/golden-matrix-2026-09-21.md)。
6 组新公开基线已获用户另行授权并完成路径清理与入库，标准报告实现完成。
7 例完整公开矩阵的独立 wheel 测量已完成：六组新案例各 79/79 完整截面，
89 截面案例为 45/46，原有缺口显式保留；六份新公开 STEP 与原始文件的 DATA
...
自己的黄金参照，required 检查也不能代替模型可用性的人类决定。
...
| FreeCAD 黄金测量 | Linux + Flatpak 1.1.3 | 全案例标准误差报告、人工结论、可重复性和无 owned-process 残留 |
