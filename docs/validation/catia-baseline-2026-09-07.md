...
已批准黄金基线；未提交 CAD 文件或批准几何公差。2026-09-16 的后续批准见本文
末节：STEP 已升级为可公开分发的首个黄金基线，数值公差仍单独待确认。
...
输入来源授权见原审计记录；生成时的授权本身不自动替代衍生 STEP 公开再分发、
黄金基线或 precision/工程公差批准。前两项已于 2026-09-16 获后续明确批准；
原始 STEP 头包含生成路径，公开夹具入库前仍应审查元数据并重新记录最终摘要。
...
## 2026-09-16 黄金基线与公开分发批准

用户明确批准 STEP SHA-256
`d9ef236cb71e2765c69badec9cc7506b4744bb5d133208dc5cc66a9988824db9`
作为首个 CATIA 黄金基线，并批准把该 STEP 随公开测试夹具分发。该批准覆盖本页
记录的输入来源、CATIA P3 V5-6R2020 生成环境和当前文件字节；公开入库时仍须先
清理原始 STEP 头中的本机生成路径等元数据，再记录清理后文件的新摘要，且不得
把新摘要冒充为本页已批准的原始摘要。

此项批准只解除首个黄金文件的来源与再分发阻塞，不自动批准 STEP writer 设置、
软件回归阈值、制造公差或把 request v2 提升为最终黄金输出。相应数值仍由
[precision 与 CATIA v2 对照提案](freecad-precision-proposal-2026-09-16.md)单独治理。
